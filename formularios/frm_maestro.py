...
class FormularioMaestro(tk.Frame):

    def __init__(self, master=None):
        super().__init__(master)
        self.master = master
        self.algoritmo = tk.IntVar(value=1)
        # no usamos title/config_window porque esto es un Frame dentro del root
        self.paneles()
        self.controles_barra_superior()
        self.tabla_resultados()
        self.resultados = []

    # removed config_window and menu-related methods (handled by principal)
    ...
    def controles_barra_superior(self):
        ...
        etiqueta_titulo = tk.Label(self.barra_superior, text="Calculadora de Números Aleatorios", bg=COLOR_BARRA_SUPERIOR, fg="white", font=fuente)
        etiqueta_titulo.pack(pady=10)

        semilla1_titulo = tk.Label(self.barra_controles, text="Semilla 1:", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        semilla1_titulo.grid(row=0, column=0, padx=10, pady=5)

        semilla2_titulo = tk.Label(self.barra_controles, text="Semilla 2:", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        semilla2_titulo.grid(row=0, column=1, padx=10, pady=5)

        constante_titulo = tk.Label(self.barra_controles, text="Constante:", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        constante_titulo.grid(row=0, column=2, padx=10, pady=5)

        algoritmo_titulo = tk.Label(self.barra_controles, text="Algoritmo:", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        algoritmo_titulo.grid(row=0, column=3, padx=10, pady=5)

        n_titulo = tk.Label(self.barra_controles, text="N:", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        ...
        # usar el IntVar ya inicializado en __init__
