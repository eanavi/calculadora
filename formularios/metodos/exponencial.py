...
class ExponencialFrame(tk.Frame):
    def __init__(self, master=None):
        super().__init__(master, bg=COLOR_CUERPO_PRINCIPAL)
        self.values = []
        self.crear_paneles_pestaña()
        self.crear_control_exponencial()
        self.crear_resultados_exponencial()
    ...
    def crear_resultados_exponencial(self):
        # Espacio para el canvas de matplotlib
        ...
        # Mostrar un gráfico vacío inicial
        self.ax.bar([], [])
        self.ax.set_title("Distribución Exponencial (Discreta por truncamiento)")
    ...
    def generar_y_graficar(self):
        try:
            ...
            messagebox.showerror("Error", "Ingrese valores numéricos válidos (Tasa flotante, N entero).")
            return

        if n <= 0 or lamda <= 0:
            messagebox.showerror("Error", "N y Tasa (λ) deben ser mayores que 0.")
            return

        self.values = []
        for _ in range(n):
            # Generación Exponencial: x = - (1/λ) * ln(R)
            r = random.random()
            while r == 0:
                r = random.random()
                
            x_exponencial_continuo = - (1.0 / lamda) * math.log(r)
            
            # Para obtener una variable DISCRETA se trunca el valor
            val = math.floor(x_exponencial_continuo)
            self.values.append(val)

        # Graficar
        self.graficar()
        messagebox.showinfo("Generación completa", f"Se generaron {n} valores aleatorios (Exponencial, discretos por truncamiento).")
    ...
    def graficar(self):
        ...
        self.ax.set_xticks(x_vals) # Asegura que las etiquetas x sean los valores discretos
        self.ax.set_title(f"Distribución Exponencial (λ={self.entry_lambda.get()})")
