...
class NormalFrame(tk.Frame):
    def __init__(self, master=None):
        ...
        self.crear_control_normal()
        self.crear_resultados_normal()
    ...
    def crear_control_normal(self):
        fuente2 = font.Font(family="Arial", size=10)
        
        # Mu (Media)
        lbl_mu = tk.Label(self.control_frame, text="Media (μ):", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        lbl_mu.grid(row=0, column=0, padx=8, pady=8, sticky="e")
        self.entry_mu = tk.Entry(self.control_frame, width=12)
        self.entry_mu.grid(row=0, column=1, padx=8, pady=8)
        self.entry_mu.insert(0, "50.0")

        # Sigma (Desviación Estándar)
        lbl_sigma = tk.Label(self.control_frame, text="Desv. Est. (σ):", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        lbl_sigma.grid(row=0, column=2, padx=8, pady=8, sticky="e")
        self.entry_sigma = tk.Entry(self.control_frame, width=12)
        self.entry_sigma.grid(row=0, column=3, padx=8, pady=8)
        self.entry_sigma.insert(0, "10.0")
    ...
    def crear_resultados_normal(self):
        ...
        self.ax.set_title("Distribución Normal (Discreta por truncamiento)")
    ...
    def generar_y_graficar(self):
        ...
        if n <= 0 or sigma <= 0:
            messagebox.showerror("Error", "N debe ser > 0 y Desv. Est. (σ) debe ser > 0.")
            return

        self.values = []
        for _ in range(n):
            # Generación Normal (continua)
            x_normal_continuo = random.gauss(mu, sigma)
            
            # Discretización por truncamiento
            val = math.floor(x_normal_continuo)
            self.values.append(val)

        self.graficar()
        messagebox.showinfo("Generación completa", f"Se generaron {n} valores aleatorios (Normal).")
    ...
    def graficar(self):
        ...
        self.ax.set_title(f"Distribución Normal (μ={self.entry_mu.get()}, σ={self.entry_sigma.get()})")
