...
class PoissonFrame(tk.Frame):
    def __init__(self, master=None):
        super().__init__(master, bg=COLOR_CUERPO_PRINCIPAL)
        self.values = []
        self.crear_paneles_pestaña()
        self.crear_control_poisson()
        self.crear_resultados_poisson()
    ...
    def crear_control_poisson(self):
        ...
        self.entry_lambda.insert(0, "3.0")
    ...
    def crear_resultados_poisson(self):
        ...
        self.ax.set_title("Distribución de Poisson")
        self.ax.set_xlabel("Número de Ocurrencias")
    ...
    def generar_y_graficar(self):
        ...
        self.values = []
        # Método de la transformación inversa (basado en la propiedad que -ln(R) sigue exp(1))
        e_lambda = math.exp(-lamda)
        ...
        for _ in range(n):
            ...
            while p > e_lambda:
                p *= random.random()
                x += 1
            # La simulación termina con x-1
            self.values.append(x - 1)

        self.graficar()
        messagebox.showinfo("Generación completa", f"Se generaron {n} valores aleatorios (Poisson).")
    ...
    def graficar(self):
        ...
        self.ax.set_title(f"Distribución de Poisson (λ={self.entry_lambda.get()})")
        self.ax.set_xlabel("Número de Ocurrencias")
        self.ax.set_ylabel("Frecuencia")
        self.fig.tight_layout()
        self.canvas.draw()