...
class WeibullFrame(tk.Frame):
    def __init__(self, master=None):
        super().__init__(master, bg=COLOR_CUERPO_PRINCIPAL)
        self.values = []
        self.crear_paneles_pestaña()
        self.crear_control_weibull()
        self.crear_resultados_weibull()
    ...
    def crear_control_weibull(self):
        fuente2 = font.Font(family="Arial", size=10)
        
        # Alpha (Escala)
        lbl_alpha = tk.Label(self.control_frame, text="Alfa (Escala, α):", bg=COLOR_CONTROLES, fg="white", font=fuente2)
        ...
        # Beta (Forma)
        lbl_beta = tk.Label(self.control_frame, text="Beta (Forma, β):", bg=COLOR_CONTROLES, fg="white", font=fuente2)
    ...
    def crear_resultados_weibull(self):
        ...
        self.ax.set_title("Distribución Weibull (Discreta por truncamiento)")
    ...
    def generar_y_graficar(self):
        ...
        if n <= 0 or alpha <= 0 or beta <= 0:
            messagebox.showerror("Error", "N, Alfa (Escala) y Beta (Forma) deben ser mayores que 0.")
            return

        self.values = []
        for _ in range(n):
            # Generación Weibull (continua)
            x_weibull_continuo = random.weibullvariate(alpha, beta)
            
            # Discretización por truncamiento
            val = math.floor(x_weibull_continuo)
            self.values.append(val)

        self.graficar()
        messagebox.showinfo("Generación completa", f"Se generaron {n} valores aleatorios (Weibull).")
    ...
    def graficar(self):
        ...
        self.ax.set_title(f"Distribución Weibull (α={self.entry_alpha.get()}, β={self.entry_beta.get()})")
